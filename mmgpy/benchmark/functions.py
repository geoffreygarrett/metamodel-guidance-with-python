...
class TestFunction:
    ...
    def normed_call(self, *args):
        _args = [args[i] * (self._bounds[1][i] - self._bounds[0][i]) +
                 self._bounds[0][i] for i in range(len(args))]
    ...
    def __call__(self, *args):
        return self._f(*args) if not self.norm_domain else self.normed_call(
            *args)
    ...
    def norm_bounds(self):
        return np.array([[0] * 2, [1] * 2]) if self._n is None else np.array(
            [[0] * self._n, [1] * self._n])
...
class Rastrigin(TestFunction):
    ...
    def __init__(self, n):
        ...
        self._n = n
        self._f = lambda *x: 10 * len(x) + np.sum(
            [x_i ** 2 - 10 * np.cos(2 * np.pi * x_i) for x_i in x], axis=0)
...
class Ackley(TestFunction):
    ...
    def __init__(self):
        ...
        self._f = lambda x, y: -20 * np.exp(
            -0.2 * np.sqrt(0.5 * (x ** 2 + y ** 2))) - np.exp(
...
class Rosenbrock(TestFunction):
    ...
    def __init__(self, n):
        ...
        self._a = lambda x_i_p_1, x_i: 100 * (x_i_p_1 - x_i ** 2) ** 2 + (
                1 - x_i) ** 2
        self._f = lambda *x: np.sum(
            [self._a(x_i_p_1, x_i) for x_i_p_1, x_i in zip(x[1:], x[:-1])],
            axis=0)
        self._n = n
...
class Beale(TestFunction):
    ...
    def __init__(self):
        ...
        self._f = lambda x, y: (1.5 - x + x * y) ** 2 + (
...
class BukinN6(TestFunction):
    def __init__(self):
        ...
        self._f = lambda x, y: 100 * np.sqrt(
            np.abs(y - 0.01 * x ** 2)) + 0.01 * np.abs(x + 10)
...
class LeviN13(TestFunction):
    def __init__(self):
        ...
        self._f = lambda x, y: (np.sin(3 * np.pi * x) ** 2 + (x - 1) ** 2 * (
...
class ThreeHumpCamel(TestFunction):
    def __init__(self):
        ...
        self._optimum = [([0.0, 0.0], 0.0)]
        self._f = (lambda x,
                          y: 2 * x ** 2 - 1.05 * x ** 4 + x ** 6 / 6 + x * y + y ** 2)
...
class Easom(TestFunction):
    ...
    def __init__(self):
        ...
        self._f = lambda x, y: - np.cos(x) * np.cos(y) * np.exp(
            -((x - np.pi) ** 2 + (y - np.pi) ** 2))
...
class CrossInTray(TestFunction):
    def __init__(self):
        ...
        self._f = lambda x, y: -1e-4 * (
                np.abs(np.sin(x) * np.sin(y) * np.exp(np.abs(
                    100 - np.sqrt(x ** 2 + y ** 2) / np.pi))) + 1) ** 1e-1
...
class Eggholder(TestFunction):
    ...
    def __init__(self):
        ...
        self._f = lambda x, y: - (y + 47) * np.sin(
            np.sqrt(np.abs(x / 2. + (y + 47)))) - x * np.sin(
...
class HolderTable(TestFunction):
    ...
    def __init__(self):
        ...
        self._bounds = np.array([
            ...
            [+10] * 2])
        self._optimum = [([512, 404.2319], -959.6407)]
        self._f = lambda x, y: - np.abs(np.sin(x) * np.cos(y) * np.exp(
...
class SchafferN2(TestFunction):
    ...
    def __init__(self):
        ...
        self._optimum = [([0.0, 0.0], 0.0)]
        self._f = lambda x, y: 0.5 + (np.sin(x ** 2 - y ** 2) ** 2 - 0.5) / (
                1 + 0.001 * (x ** 2 + y ** 2)) ** 2
...
class StyblinskiTang(TestFunction):
    ...
    def __init__(self, n):
        ...
        self._bounds = np.array([
            [-5] * n,
            [+5] * n])
        # Optimum is not phrased as below. Check Wikipedia for actual test
        # (This is only approx to 5th decimal place).
        self._optimum = [([-2.903535] * n, -39.166165 * n)]
        self._f = lambda x_vec: np.sum(
            [(x_i ** 4 - 16 * x_i ** 2 + 5 * x_i) / 2.0 for x_i in x_vec],
            axis=0)
...
class GarrettLagadec(TestFunction):
    ...
    def __init__(self, n, lam=0.5):
        ...
        def f(x_vec):
            return np.max(
                [np.min([np.abs(x_i - lam), np.abs(x_i + lam)], axis=0) for x_i
                 in x_vec], axis=0)
