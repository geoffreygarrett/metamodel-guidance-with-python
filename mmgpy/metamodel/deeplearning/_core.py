    ...
    def _train(self, verbose,
        ...
        for epoch in range(self._epoch, self._num_epochs):
            ...
            if verbose:
                if verbose is True:
                    print(
                        f'Epoch [{self._epoch + 1:^5}/{self._num_epochs:^5}]  |  Train Loss: {train_loss_item: 8.6f}  |  Validation Loss: {validation_loss_item: 8.6f}  |  lr: {learning_rate: 6.5f}')
                elif type(verbose) is int:
                    if self._epoch % verbose == 0:
                        print(
                            f'Epoch [{self._epoch + 1:^5}/{self._num_epochs:^5}]  |  Train Loss: {train_loss_item: 8.6f}  |  Validation Loss: {validation_loss_item: 8.6f}  |  lr: {learning_rate: 6.5f}')
            ...
            if early_stopping is not None:
                # early_stopping needs the validation loss to check if it has decreased,
                # and if it has, it will make a checkpoint of the current model
                early_stopping(validation_loss_item, train_loss_item)
