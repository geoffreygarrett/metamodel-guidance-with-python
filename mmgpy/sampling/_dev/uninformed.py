...
def grid_sample_domain(bounds, n_sample_dim, flatten=True, endpoint=True):
    ...
    _return = np.array(
        [_return_i.flatten() for _return_i in _return.reshape(len(bounds[0]), -1)]) if flatten else _return
    return _return
...
def halton_sample_domain(bounds, n_samples):
    ...
    return (halton_sample * (bounds[1, :] - bounds[0, :]) + bounds[0, :]).T


def multivariate_normal_sample_point(point, covariance, size, seed=101):
    np.random.seed(seed)
    return (np.random.multivariate_normal(point, cov=covariance, size=size)).T
