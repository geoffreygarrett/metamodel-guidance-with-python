...
void MdlOutputs(int_T tid)
{
  ...
  if (ssIsSampleHit(rtS, 1, 0)) {
    ...
    rtB.vel = (rtDW.clockTickCounter < rtP.PulseGenerator_Duty) &&
      (rtDW.clockTickCounter >= 0) ? rtP.PulseGenerator_Amp : 0.0;
     ...
     */
    rtB.Product_c = rtB.vel * rtP.reaction_time_Value;

    /* Math: '<S12>/Square' */
    rtB.Square = rtB.vel * rtB.vel;
  }
  ...
  if (ssIsSampleHit(rtS, 1, 0)) {
     ...
     */
    rtB.RelationalOperator1_c = (rtB.vel >= rtP.Constant_Value_k);

    /* Logic: '<S13>/Logical Operator' */
    rtB.LogicalOperator_l = (rtB.RelationalOperator_p &&
     ...
     */
    rtB.RelationalOperator3 = (rtB.vel < rtP.Constant2_Value);
    ...
    if (rtB.LogicalOperator2) {
      ...
      rtB.cyclist_power_density = rtB.Diff * rtB.vel;
  ...
  if (ssIsSampleHit(rtS, 1, 0)) {
     ...
     */
    rtB.Exp = rt_powd_snf(rtB.vel, rtP.Constant_Value_j);

    /* Abs: '<S16>/Abs' */
    rtB.Abs_h = fabs(rtB.Exp);
    rs_bicycle_MovingMaximum3(rtB.Abs_h, &rtB.MovingMaximum3,
      &rtDW.MovingMaximum3);
  }
...
void MdlDerivatives(void)
{
  ...
  _rtXdot->Integrator_CSTATE = rtB.vel;
...
void MdlInitializeSizes(void)
{
  ...
  ssSetNumBlocks(rtS, 105);            /* Number of blocks */
  ssSetNumBlockIO(rtS, 67);            /* Number of block outputs */
  ssSetNumBlockParams(rtS, 41);        /* Sum of parameter "widths" */
}
...
SimStruct * rs_bicycle(void)
{
  ...
  ssSetChecksumVal(rtS, 0, 2204191952U);
  ssSetChecksumVal(rtS, 1, 2930770560U);
  ssSetChecksumVal(rtS, 2, 3378328187U);
  ssSetChecksumVal(rtS, 3, 2970185107U);
  return rtS;
}
